...
export const PaymentSuccess = () => {
    ...
    useEffect(() => {
        const validatePaymentAndGetReservation = async () => {
          try {
            const validationResponse = await axios.post(
              `http://localhost:8080/api/checkout/validate-payment/${checkoutId}`,
              {},
              {
                headers: {
      ...
      const downloadFacture = async () => {
        navigate("../suivi-facture")
        // try {
          
      
        //   const response = await fetch(
        //     `http://localhost:8080/api/checkout/facture/${4}`, 
        //     {
        //       headers: {
        //         'Authorization': `Bearer ${token}`
        //       }
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.content}>
          ...
          <button onClick = {() => navigate("../booking")}style={styles.tertiaryButton}>
            Plus tard
          </button>
        </div>
      </div></div>
