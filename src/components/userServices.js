...
import "../styles/userServicesList.css"
import { BasePage } from "./BasePage"
export const UserServices = () => {
  const { token, user } = useAuth();
  const { reservationId,checkoutId } = useParams();
    ...
    const fetchConsumedServices = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/api/reservation-services/during-stay/${reservationId}`,
          {
            headers: {
  ...
  const handlePayment = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/checkout/payer/${checkoutId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handlePayPalPayment = async () => {
    try {
    const response = await axios.get(`http://localhost:8080/api/checkout/paypal-payment/${reservationId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
